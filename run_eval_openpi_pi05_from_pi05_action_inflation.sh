#!/usr/bin/env bash
# Evaluate OpenPI π0.5 (openpi_p0.5) on LIBERO using pre-recorded action-inflation attack prompts from openpi_pi05.
#
# Reads original_instruction / perturbed_instruction from the openpi_pi05 action
# inflation attack record, cross-checks original_instruction against LIBERO ground
# truth, and runs openpi_pi05 twice per episode (baseline + attack).
#
# Output goes to outputs/eval_result/ in the same JSON format as the recorded replay.
#
# Usage:
#   bash run_eval_openpi_pi05_from_pi05_action_inflation.sh                 # GPU 0
#   bash run_eval_openpi_pi05_from_pi05_action_inflation.sh --gpu 2         # GPU 2
#   bash run_eval_openpi_pi05_from_pi05_action_inflation.sh --no-aggregate  # skip aggregation
set -euo pipefail
...
echo "========================================"
echo "  OpenPI π0.5 Action Inflation Replay Attack Evaluation"
echo "  Victim:  ${VICTIM}"
...
if [[ "$DO_AGGREGATE" == true ]] && command -v python &>/dev/null; then
  ...
  python aggregate_replay_results.py \
    ...
    --output "$OUTPUT_DIR/openpi_pi05_eval_summary.json" \
    2>&1 | tee "${OUTPUT_DIR}/aggregation_openpi_pi05.log" || true
fi

echo ""
echo "========================================"
echo "  OpenPI π0.5 action inflation evaluation complete."
echo "  Report:   ${OUTPUT_DIR}/replay_action_inflation_${VICTIM}_from_openpi_pi05.json"
echo "  Summary:  ${OUTPUT_DIR}/openpi_pi05_eval_summary.json"
echo "========================================"
