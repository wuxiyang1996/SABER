#!/usr/bin/env bash
# Record all attack-agent outputs given the initial LIBERO instructions (action_inflation objective).
#
...
#
# Each task runs 8 trials; only the trial with the most attack action steps
...
#   action_inflation_<model>.json      — structured JSON with per-episode records including
...
#
# Usage:
#   bash run_record_agent_outputs_action_inflation.sh                        # all 8 models
#   bash run_record_agent_outputs_action_inflation.sh openvla                # single model
#   bash run_record_agent_outputs_action_inflation.sh openvla lightvla       # two in parallel
set -euo pipefail
