#!/usr/bin/env bash
# Evaluate OpenPI π0 (open_pi0_libero) on LIBERO using pre-recorded constraint-violation attack prompts from openpi_pi05.
#
...
# truth, and runs openpi_pi0 twice per episode (baseline + attack).
#
# Output goes to outputs/eval_result/ in the same JSON format as the recorded replay.
#
# Usage:
#   bash run_eval_openpi_pi0_from_pi05_constraint_violation.sh                 # GPU 0
#   bash run_eval_openpi_pi0_from_pi05_constraint_violation.sh --gpu 2         # GPU 2
#   bash run_eval_openpi_pi0_from_pi05_constraint_violation.sh --no-aggregate  # skip aggregation
set -euo pipefail
...
echo "========================================"
echo "  OpenPI π0 Constraint Violation Replay Attack Evaluation"
echo "  Victim:  ${VICTIM}"
...
echo ""
echo "========================================"
echo "  OpenPI π0 constraint violation evaluation complete."
